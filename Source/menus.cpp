...
void individualtournamentmenu(unsigned int level, unsigned int id) {
	...
	do {
		...
		while (repeatmenu) {
			...
			cout << " This Team| Adversary" << endl;
			vector<pair<unsigned int, unsigned int>> vector_results = tournament->getResults();
			for (unsigned int i = 0; i < vector_results.size(); i++) {
				cout << setw(10) << vector_results[i].first << "|  " << vector_results[i].second<<endl;
			}
			cout << "\nPlayers that participated in this training:" << endl;
...
void editresultstournament(unsigned int level, Event * tournament) {
	...
	vector<pair<unsigned int, unsigned int>> results;//vector that will store the results typed by the user
	...
	while (repeatloop) {
		...
		while (repeatquestion && repeatloop) {
			...
			if (second_score == "f") {
				...
				repeatquestion = false;
				results.push_back(pair<unsigned int, unsigned int>(convint(first_score), convint(second_score)));
			}
		}
		game_counter++;
	}
